...
poids_ideal <- (biometry$taille - 100 + 6 * biometry$tour_poignet)/2
poids_ideal
```
...
biometry_H <- smutate(biometry, rth_levels = case_when(rth >= 0.95 ~ "Hausse du risque", 
  rth < 0.95 ~ "normal"))
...
biometry_F <- smutate(biometry, rth_levels = case_when(rth >= 0.85 ~ "Hausse du risque", 
  rth < 0.85 ~ "normal"))
...
biometry_H%>.%
  sfilter(.,genre=="H")%>.%
  smutate(.,biometry_H$imc <- biometry$masse_corr / ((biometry$taille/100) ^ 2))
biometry_H<-smutate(biometry, imc_levels = case_when(
  imc<18.5 ~ "sous-poids",
