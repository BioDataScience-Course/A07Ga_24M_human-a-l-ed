...
sselect(biometry, poids_ideal, age, genre) %>.%
  smutate(., age_levels = case_when(
    age <= 18 ~ "Enfant",
    age >= 18 & age < 29 ~ "Jeune adulte",
    age >= 29 & age < 60 ~ "Adulte",
    age >= 60 ~ "Senior")) %>.%
tabularise(.)
```
...
table_F<-table(IMC=biometry_F$imc_levels,rth_Femme=biometry_F$rth_levels)
tabularise(table_F)
table_H<-table(IMC=biometry_H$imc_levels,rth_Homme=biometry_H$rth_levels)
tabularise(table_H)
