...
```{r plot_chi2_1}
#Tableau de la 2e métrique (poids idéal calculé, formule de Lorentz avec correction)
sselect(biometry, poids_ideal, age, genre, masse_corr) %>.%
  smutate(., masse_corr_levels = case_when(
    masse_corr < 50 ~ "Très mince",
    masse_corr >= 50  & masse_corr < 60 ~ "Mince",
    masse_corr >= 60 & masse_corr < 75 ~ "Normal",
    masse_corr >= 75 & masse_corr < 90 ~ "Surpoids",
    masse_corr > 90 ~ "Surpoids")) %>.%
tabularise(.)
```
...
```{r plot_anova1_1}
chart(biometry, fast_food ~ sucre) +
  geom_violin() +
...
-   $$H_0 : var(jamais) = var(rarement) = var(régulièrement) = var(souvent) $$
-   $$H_1 : \exists(i, j)\ tel\ que\ var_i≠var_j$$
...
geom_hline(yintercept = mean(biometry$masse_corr), color = "blue", linetype = "dashed", size = 1)
```

<!--% Hypothèse nulle et alternative -->

-   $$H_0 : µ(omnivore) = µ(carnivore) = µ(végétarien)$$

-   $$ H_1 : \exists(i, j)\ tel\ que\ µ_i≠µ_j$$

```{r anova2_1}
anova(biometry_anova<-lm(data=biometry, masse_corr ~ alimentation))|>tabularise()

```

<!--% Hypothèse nulle et alternative -->

-   $$H_0 : var(omnivore) = var(carnivore) = var(végétarien) (Homoscédasticité) $$
...
<!--% Interprétation statistique et biologique -->
Dans un premier temps Dans un second temps si on regarde le test ANOVA, la valeur de P est inférieure à celle de alpha, on rejette donc H_0 au seuil de alpha 5% : les moyennes des 3 groupes ne diffèrent pas de façon significative (ANOVA, ddl = 2, P = 0.371).
Dans un second temps si l'on regarde le test de Bartlett, on voit qu'il n'y a pas homoscédasticité (au moins une varience diffère des autres) ce qui signifie que le test ANOVA ne sera pas concluant.
