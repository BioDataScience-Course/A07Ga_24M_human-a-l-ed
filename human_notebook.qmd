...
biometry%>.%
  ...
  bmi >= 30 ~ "obese"),
     bmi_cat = factor(bmi_cat,
                 levels = c("underweight", "normal weight", "overweight", "obese"),
                 ordered = TRUE) ) %>.%
  sselect(., masse_grasse, lbm , masse_corr, genre,bmi_cat) %>.%
  sgroup_by(.,bmi_cat, genre )%>.%
  ssummarise(., 
    mean = fmean(masse_grasse),
    var = fvar(masse_grasse),
    n = fn(masse_grasse)
    ) ->
  
  graisse_tab
tabularise(graisse_tab)
```

<!--% Hypothèse nulle et alternative -->

-   $$H_0 : Il \ n'y \ a \ pas \ d'effet \ significatif \ du \ BMI \ et \ du \ genre \ sur \ la \ masse \ grasse $$

```{=html}
<!-- -->
```
-   $$H_0 : Il \ y \ a \ un \ effet \ significatif \ du \ BMI \ et \ du \ genre \ sur \ la \ masse \ grasse $$
...
-   $$H_0 : Il \ y \ a \ un \ effet \ significatif \ du \ type \ d'intolérance \ sur \ la \ masse $$
...
α de 5% que nous ne pouvons pas rejeter H_0 dans les trois cas (p \> α) \# Discussion et conclusions.

On ne peut donc pas conclure qu'il y aie de différence significative entre la masse corrigée au seuil α de 5% à la fois en fonction de l'intolérence au lactose (*F* = 0,0482, ddl = 1 et 140, valeur *p* = 0,8265) et de l\'intolérance au gluten (*F* = 2,6189, ddl = 1 et 140, valeur *p* = 0,1078).

Le graphique semble montrer qu'il y aie interaction entre les valeurs, le test anova deux facteurs montre le contraire (ligne "intolerance_lactose:intolerance_gluten").
