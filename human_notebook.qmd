...
```{r student3}
chart(biometry, ~ masse_corr %fill=% sucre )+
  geom_histogram(bins=20)
biogenre <- ssummarise(sgroup_by(biometry, sucre,genre),
  "n"       = fn(masse_corr),
  "poids moyen"=fmean(masse_corr)
  )
tabularise(biogenre)
```
...
biometry$masse_rec <- cut(biometry$masse_corr,
  include.lowest = TRUE,
  right = FALSE,
  dig.lab = 4,
...
-   Le poids idéal (métrique calculée) des hommes est suppérieur à celui des femmes
